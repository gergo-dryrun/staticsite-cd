...
PARAMETERS:=parameters/$(STACK).json
AWS_REGION:=$(REGION)
AWS_PROFILE:=default

create:
	@which aws || pip install awscli
	aws cloudformation create-stack --stack-name $(STACK_NAME) --template-body file://`pwd`/$(TEMPLATE) --parameters file://`pwd`/$(PARAMETERS) --capabilities CAPABILITY_NAMED_IAM --profile $(AWS_PROFILE) --region $(AWS_REGION)

update:
	aws cloudformation update-stack --stack-name $(STACK_NAME) --template-body file://`pwd`/$(TEMPLATE) --parameters file://`pwd`/$(PARAMETERS) --capabilities CAPABILITY_NAMED_IAM --profile $(AWS_PROFILE) --region $(AWS_REGION)
...
watch2:
	 watch --interval 2 "bash -c 'make events2 | head -25'"
